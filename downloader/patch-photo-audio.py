...
# TikTok photo URLs commonly resolve to /@username/photo/<id>, not /photo/<id>.
s = s.replace("return isTikTokHost(parsed.hostname) && /^\\/photo\\/\\d+/.test(parsed.pathname);", "return isTikTokHost(parsed.hostname) && /^(?:\\/@[^/]+)?\\/photo\\/\\d+/.test(parsed.pathname);")
s = s.replace("const match = final.pathname.match(/^\\/photo\\/(\\d+)/);", "const match = final.pathname.match(/(?:^|\\/)photo\\/(\\d+)/);")

# Add a safe extractor for TikTok's slideshow soundtrack URL.
...
# Include soundtrack data in the resolved photo object.
...
# Add a bounded, redirect-safe audio fetcher and ffmpeg conversion.
...
# Return music metadata from /api/info.
...
# Use the soundtrack when the user selects audio for a photo post.
...
# Frontend: photo posts should offer both ZIP photos and MP3 soundtrack when available.
h = html.read_text(encoding='utf-8')
old = "if(detectedContentType==='photo'){\n        typeSelect.value='video'; typeSelect.disabled=true; qualitySelect.style.display='none';\n        detectedBox.textContent=`TikTok photo slideshow detected — ${data.imageCount||'multiple'} image(s) will be saved together as a ZIP.`;\n        detectedBox.classList.remove('hidden');"
new = "if(detectedContentType==='photo'){\n        clearPreview();\n        typeSelect.disabled=false;\n        qualitySelect.style.display='none';\n        typeSelect.innerHTML='<option value=\"video\">Photos (ZIP)</option><option value=\"audio\">Soundtrack (MP3)</option>';\n        typeSelect.value='video';\n        detectedBox.textContent=data.hasAudio ? `TikTok photo slideshow detected — ${data.imageCount||'multiple'} image(s) can be saved as a ZIP, or download the attached soundtrack as MP3.` : `TikTok photo slideshow detected — ${data.imageCount||'multiple'} image(s) will be saved together as a ZIP. This post has no downloadable soundtrack.`;\n        detectedBox.classList.remove('hidden');"
if old not in h:
    raise SystemExit('Photo UI target not found')
h = h.replace(old, new, 1)

old_else = "}else{\n        showPreview(data);\n        typeSelect.disabled=false;"
new_else = "}else{\n        showPreview(data);\n        typeSelect.disabled=false;\n        typeSelect.innerHTML='<option value=\"video\">Video (with sound)</option><option value=\"audio\">Sound only (MP3)</option>';"
if old_else not in h:
    raise SystemExit('Normal type UI target not found')
h = h.replace(old_else, new_else, 1)

# On a new/empty URL, restore the normal choices.
h = h.replace("clearTimeout(infoDebounce); detectedContentType='video'; detectedBox.classList.add('hidden'); clearPreview();", "clearTimeout(infoDebounce); detectedContentType='video'; detectedBox.classList.add('hidden'); clearPreview(); typeSelect.disabled=false; typeSelect.innerHTML='<option value=\"video\">Video (with sound)</option><option value=\"audio\">Sound only (MP3)</option>';", 1)

html.write_text(h, encoding='utf-8')
print('TikTok photo detection/audio patched')