...
function extractScriptJson(html, scriptId) { const exactMarker = `<script id="${scriptId}"`; let start = html.indexOf(exactMarker); if (start < 0) { const idNeedle = `id="${scriptId}"`; const idIndex = html.indexOf(idNeedle); if (idIndex < 0) return null; start = html.lastIndexOf('<script', idIndex); if (start < 0) return null; } const tagEnd = html.indexOf('>', start); if (tagEnd < 0) return null; const end = html.indexOf('</script>', tagEnd); if (end < 0) return null; const raw = html.slice(tagEnd + 1, end).trim(); if (!raw) return null; try { return JSON.parse(raw); } catch { const objectStart = raw.indexOf('{'); const objectEnd = raw.lastIndexOf('}'); if (objectStart >= 0 && objectEnd > objectStart) { try { return JSON.parse(raw.slice(objectStart, objectEnd + 1)); } catch {} } return null; } }
...
async function resolveTikTokPhoto(url) { const { finalUrl, html } = await fetchTikTokPage(url); if (!isTikTokPhotoPath(finalUrl)) return null; const final = new URL(finalUrl); const match = final.pathname.match(/^\/photo\/(\d+)/); if (!match) throw new Error('Could not determine the TikTok photo post ID.'); const postId = match[1]; const item = extractTikTokPageData(html, postId); if (!item) throw new Error('TikTok loaded the photo page but did not expose its image data to the downloader.'); const imageUrls = collectTikTokImageUrls(item); if (!imageUrls.length) throw new Error('This TikTok photo post contains no downloadable images.'); return { postId, finalUrl, title: item.imagePost?.title || item.desc || 'TikTok photo slideshow', thumbnail: imageUrls[0], uploader: item.author?.nickname || item.author?.uniqueId || null, imageUrls }; }
async function tryResolveTikTokPhoto(url) { try { return await resolveTikTokPhoto(url); } catch (error) { try { const { finalUrl } = await fetchTikTokPage(url); if (isTikTokPhotoPath(finalUrl)) throw error; } catch (secondError) { if (isTikTokPhotoPath(url)) throw error; } return null; } }
...
// ---------- Metadata ----------
app.post('/api/info', requireAuth, rateLimit, async (req, res) => { const { url } = req.body || {}; if (!url || !(await isSafeUrl(url))) return res.status(400).json({ error: 'A valid, public http(s) URL is required.' }); try { const photo = await tryResolveTikTokPhoto(url); if (photo) return res.json({ title: photo.title, thumbnail: photo.thumbnail, duration: null, uploader: photo.uploader, extractor: 'TikTok Photo', contentType: 'photo', imageCount: photo.imageUrls.length, availableHeights: [] }); const { stdout } = await runYtDlp(['-j', '--no-playlist', url]); const info = JSON.parse(stdout.trim().split('\n')[0]); return res.json({ title: info.title, thumbnail: info.thumbnail, duration: info.duration, uploader: info.uploader, extractor: info.extractor, contentType: 'video', availableHeights: [...new Set((info.formats || []).map(format => format.height).filter(Boolean))].sort((a, b) => b - a) }); } catch (error) { return res.status(422).json({ error: 'Could not read that link.', detail: truncate(error.message) }); } });
...
async function downloadExternalProvider(job) {
  if (job.type === 'audio') throw new Error('The external provider is configured for video jobs only.');
...
// ---------- Download ----------
app.post('/api/download', requireAuth, rateLimit, async (req, res) => { const { url, quality = 'best', type = 'video' } = req.body || {}; if (!url || !(await isSafeUrl(url))) return res.status(400).json({ error: 'A valid, public http(s) URL is required.' }); if (!['video', 'audio'].includes(type)) return res.status(400).json({ error: "type must be 'video' or 'audio'." }); if (activeJobCount >= MAX_CONCURRENT_JOBS) return res.status(429).json({ error: 'Server is busy processing other downloads — please try again in a moment.' }); const id = uuidv4(); const job = { id, url, quality, type, status: 'processing', title: null, thumbnail: null, filePath: null, fileSize: null, error: null, provider: null, createdAt: new Date().toISOString() }; await upsertJob(job); res.json({ jobId: id }); activeJobCount++; processJob(id).catch(async error => { const current = getJob(id); if (!current) return; current.status = 'failed'; current.error = truncate(error.message); await upsertJob(current); }).finally(() => { activeJobCount--; }); });

async function processJob(id) { const job = getJob(id); if (!job) return; const photo = await tryResolveTikTokPhoto(job.url); if (photo) { if (job.type === 'audio') throw new Error('TikTok photo posts contain images, so audio-only download is not available.'); const result = await processTikTokPhotoJob(job, id, photo); const updated = getJob(id); Object.assign(updated, result, { status: 'done', type: 'photo', provider: 'tiktok' }); await upsertJob(updated); return; }
  let info = {};
  try { const { stdout } = await runYtDlp(['-j', '--no-playlist', job.url]); info = JSON.parse(stdout.trim().split('\n')[0]); } catch {}
  const useExternal = String(process.env.VIDKRAKEN_ENABLED || '').toLowerCase() === 'true' && Boolean(process.env.VIDKRAKEN_API_KEY) && job.type === 'video';
  if (useExternal) {
    try {
      const external = await downloadExternalProvider(job);
      const updated = getJob(id); updated.status = 'done'; updated.provider = 'vidkraken'; updated.title = info.title || 'Downloaded video'; updated.thumbnail = info.thumbnail || null; updated.duration = info.duration || null; updated.sourcePlatform = info.extractor || null; updated.filePath = external.filePath; updated.fileSize = external.fileSize; await upsertJob(updated); return;
    } catch (externalError) {
      console.warn(`[external-provider] ${truncate(externalError.message)}`);
    }
  }
  const outputTemplate = path.join(DOWNLOAD_DIR, `${id}.%(ext)s`); let args; if (job.type === 'audio') { args = ['-x', '--audio-format', 'mp3', '--audio-quality', '0', '--no-playlist', '-o', outputTemplate, job.url]; } else { const format = QUALITY_FORMATS[job.quality] || QUALITY_FORMATS.best; args = ['-f', format, '--merge-output-format', 'mp4', '--no-playlist', '-o', outputTemplate, job.url]; } await runYtDlp(args); const files = fs.readdirSync(DOWNLOAD_DIR).filter(file => file.startsWith(id)); if (!files.length) throw new Error('Download finished but no output file was found.'); const finalFile = files[0]; const fullPath = path.join(DOWNLOAD_DIR, finalFile); const updated = getJob(id); updated.status = 'done'; updated.provider = 'yt-dlp'; updated.title = info.title || 'Untitled'; updated.thumbnail = info.thumbnail || null; updated.duration = info.duration || null; updated.sourcePlatform = info.extractor || null; updated.filePath = finalFile; updated.fileSize = fs.statSync(fullPath).size; await upsertJob(updated); }
