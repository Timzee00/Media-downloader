...
html_add = html_marker + '\n    <div id="previewBox" class="preview-box hidden"><img id="previewImage" alt="Media preview" referrerpolicy="no-referrer"><video id="previewVideo" controls playsinline preload="metadata" referrerpolicy="no-referrer"></video><div class="preview-meta"><div class="preview-title" id="previewTitle">Media preview</div><div>Preview only — your file is not downloaded until you press Download.</div></div></div>'
...
js_add = js_marker + "\nconst previewBox=$('#previewBox'),previewImage=$('#previewImage'),previewVideo=$('#previewVideo'),previewTitle=$('#previewTitle');\nfunction clearPreview(){ previewVideo.pause(); previewVideo.removeAttribute('src'); previewVideo.load(); previewImage.removeAttribute('src'); previewImage.style.display='block'; previewVideo.style.display='none'; previewBox.classList.add('hidden'); previewTitle.textContent='Media preview'; }\nfunction showPreview(data){ clearPreview(); if(!data) return; previewTitle.textContent=data.title||'Media preview'; const thumb=data.thumbnail; const video=data.previewUrl; if(thumb){ previewImage.onerror=()=>{ previewImage.style.display='none'; if(video){ previewVideo.src=video; previewVideo.style.display='block'; previewVideo.load(); } else { previewBox.classList.add('hidden'); } }; previewImage.src=thumb; previewBox.classList.remove('hidden'); return; } if(video){ previewImage.style.display='none'; previewVideo.style.display='block'; previewVideo.src=video; previewBox.classList.remove('hidden'); previewVideo.load(); } }"
...
print('thumbnail-based preview patched')
